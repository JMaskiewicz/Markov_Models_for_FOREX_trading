...
df['Standardized_Log_Returns'] = (df['Log_Returns'] - df['Log_Returns'].mean()) / df['Log_Returns'].std()
...
for k_regimes in range(2, 7):
    mod = MarkovRegression(df['Standardized_Log_Returns'], k_regimes=k_regimes, trend='c', switching_variance=True)
    res = mod.fit()
...
axs[1].plot(df.index, df['Standardized_Log_Returns'], label='Standardized Log Returns')
...
for state in range(best_k_regimes):
    axs[2].plot(df.index, smoothed_probs[state], label=f'State {state+1}')

axs[2].set_title('Probability of Being in Each State Over Time (MSM)')
...
axs[0].plot(df.index, rolling_mean, label='Rolling Mean (50 days) ')
...
axs[1].plot(df.index, rolling_variance, label='Rolling Variance (50 days)')
